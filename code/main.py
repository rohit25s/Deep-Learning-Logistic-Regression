...
def visualize_features(X, y):
    ...
    idx1 = np.where(y == 1)
    idxneg1 = np.where(y == -1)
    X1 = X[idx1]
    Xneg1 = X[idxneg1]

    plt.scatter(X1[:, 0], X1[:, 1], c='r')
    plt.scatter(Xneg1[:, 0], Xneg1[:, 1], c='g')
    plt.xlabel('symmetry')
...
....'''
    ...
    for (i, sample) in enumerate(X):
        xc = np.matmul(W.T, sample)
        yc = 1 / (1 + np.exp(np.sum((-1)*np.matmul(W.T, sample))))
        if y[i] == 1:
...
def main():
    ...
    ### YOUR CODE HERE
    logisticR_classifier = logistic_regression(learning_rate=0.2, max_iter=100)
    logisticR_classifier.fit_BGD(train_X, train_y, 10)
    print(logisticR_classifier.get_params())
    print(logisticR_classifier.score(train_X, train_y))
    ### END YOUR CODE
    ...
    visualize_result(train_X, train_y, logisticR_classifier.get_params())
    ...
    # Explore different hyper-parameters.
    ### YOUR CODE HERE
    logisticR_classifier_multiclass = logistic_regression_multiclass(learning_rate=0.7, max_iter=100,  k= 3)
    logisticR_classifier_multiclass.fit_BGD(train_X, train_y, 1)
    print(logisticR_classifier_multiclass.get_params())
    print(logisticR_classifier_multiclass.score(train_X, train_y))
    ### END YOUR CODE
